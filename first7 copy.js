...
app.post('/api/watchlist/add', async (req, res) => {
  ...
  if (!symbol || typeof symbol !== 'string') {
    return res.status(400).json({ error: 'Missing symbol or invalid symbol.' });
  }
  ...
  try {
    // 先查一遍，看看是否已经存在
    const [existing] = await db.execute('SELECT * FROM stockwatchlist WHERE symbol = ?', [upperSymbol]);
    if (existing.length > 0) {
      return res.status(409).json({ error: 'Symbol already in watchlist.' });
    }

    // 不存在才插入
    await db.execute('INSERT INTO stockwatchlist (symbol) VALUES (?)', [upperSymbol]);
    res.json({ success: true, message: `${upperSymbol} added to watchlist.` });
  } catch (err) {
    console.error(err);
    res.status(500).json({ error: 'Failed to add the watchlist.' });
  }
});
      ...
      try {
        ...
        if (Math.abs(percent) >= threshold) {
          ...
          alerts.push({
            ...
            direction,
            message: `${symbol} has ${direction} more than ${threshold}% (${sign}${percent.toFixed(2)}%)`
          });
        }
      } catch (e) {
